...
"""
NS-3 Experiment Manager (Proteus Conga Edition)
...
def main():
    ...
    parser.add_argument('tag', help='Experiment tag (label), e.g., "dcqcn_alpha_0.5"')
    parser.add_argument('--desc', default='', help='Brief description of the experiment')
    args = parser.parse_args()

    # Setup paths
    # Current script directory: ns-3.19/mix/Reproduction/Proteus/Proteus_Conga/
    ...
    csv_dir = os.path.normpath(os.path.join(EXP_DIR, "analyze_csv"))

    analysis_cmd = f"bash {analysis_script} -o {output_dir} -f {figures_dir} -c {csv_dir}"
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write("*(Describe key parameter changes here, e.g., DCQCN Alpha, G, etc.)*\n\n")
