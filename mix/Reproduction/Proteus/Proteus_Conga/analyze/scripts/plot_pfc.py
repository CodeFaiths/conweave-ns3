...
# Default paths
DEFAULT_PFC_FILE = os.path.join(BASE_DIR, "..", "..", "output", "out_pfc.txt")
...
def main():
    ...
    parser.add_argument(
        "--input",
        default=DEFAULT_PFC_FILE,
        help="Input pfc file path"
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--include",
        nargs="*",
        help="Optional list of node labels to keep. e.g., S2 S5",
    )
    parser.add_argument(
        "--time",
        nargs=2,
    ...
    out_png = os.path.join(args.output, "pfc_plot.png")

    data = parse_pfc(args.input)
    if not data:
